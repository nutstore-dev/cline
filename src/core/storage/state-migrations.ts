...
export async function migrateLegacyApiConfigurationToModeSpecific(context: vscode.ExtensionContext) {
	try {
		...
		const nutstoreModelId = await context.globalState.get("nutstoreModelId")
		const nutstoreModelInfo = await context.globalState.get("nutstoreModelInfo")
		...
		if (planActSeparateModelsSetting === false) {
			...
			if (nutstoreModelId !== undefined) {
				await context.globalState.update("planModeNutstoreModelId", nutstoreModelId)
				await context.globalState.update("actModeNutstoreModelId", nutstoreModelId)
			}
			if (nutstoreModelInfo !== undefined) {
				await context.globalState.update("planModeNutstoreModelInfo", nutstoreModelInfo)
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const nutstoreAccessToken = await context.secrets.get("nutstoreAccessToken")
